...
serve(async (req) => {
  ...
  try {
    ...
    if (!test_date) {
      return new Response(
        JSON.stringify({ error: 'test_date is required' }),
        { 
    ...
    const { data: employeeRecords, error: employeeUpdateError } = await supabase.rpc(
      'update_compliance_statuses_with_date',
      { test_date }
    );
    ...
    const { data: clientRecords, error: clientUpdateError } = await supabase.rpc(
      'update_client_compliance_statuses_with_date',
      { test_date }
    );
