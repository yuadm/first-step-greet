...
serve(async (req) => {
  ...
  try {
    ...
    for (const type of complianceTypes as ComplianceType[]) {
      console.log(`Processing compliance type: ${type.name} (${type.frequency})`);
      ...
      if (shouldGenerate) {
        console.log(`Generating records for ${type.name} - ${periodIdentifier}`);
        ...
        if (generateError) {
          console.error(`Error generating records for ${type.name}:`, generateError);
        } else {
          const recordsCreated = result || 0;
          totalRecordsCreated += recordsCreated;
          console.log(`Created ${recordsCreated} records for ${type.name}`);
        }
      }
    }

    // Update compliance statuses (check for overdue items)
    console.log('Updating compliance statuses...');
    const { data: updateResult, error: updateError } = await supabase
      .rpc('update_compliance_statuses');

    if (updateError) {
      console.error('Error updating compliance statuses:', updateError);
    } else {
      statusUpdates = updateResult || 0;
      console.log(`Updated ${statusUpdates} overdue records`);
    }
    ...
    const response = {
      ...
      message: `Processed ${complianceTypes.length} compliance types. Created ${totalRecordsCreated} new records, updated ${statusUpdates} statuses.`
    };
