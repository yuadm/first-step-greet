...
-- Fix document_tracker table structure for JSONB array design
-- The table was refactored to store documents in a JSONB array, but old columns still have NOT NULL constraints

-- Make old individual document columns nullable since they're now in the JSONB array
ALTER TABLE document_tracker 
  ALTER COLUMN document_type_id DROP NOT NULL,
  ALTER COLUMN expiry_date DROP NOT NULL,
  ALTER COLUMN status DROP NOT NULL;

-- Create or replace the upsert function to work with the new structure
...
-- Create or replace the delete function
...
BEGIN
  ...
  FOR i IN 0..jsonb_array_length(v_documents) - 1 LOOP
    IF (v_documents->i)->>'id' != p_document_id THEN
      v_updated_documents := v_updated_documents || jsonb_build_array(v_documents->i);
    END IF;
  END LOOP;
  
  -- Update the tracker with filtered documents
  UPDATE document_tracker
  SET 
    documents = v_updated_documents,
    updated_at = NOW()
  ...
  RETURN jsonb_build_object('success', true, 'tracker_id', v_tracker_id);
END;
$$;

-- Add helpful comment
COMMENT ON TABLE document_tracker IS 'Stores employee documents in a JSONB array. Each employee has one row with all their documents in the documents column.';
